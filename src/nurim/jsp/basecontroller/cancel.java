...
public class cancel extends BaseController {
	...
	public String doRun(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			//로그인 정보로 주문리스트 받기
			orderList = orderService.selectOrderList2(member);
			logger.debug("orderList >> " + orderList);
			//배송상활별 카운트 갯수 등록
			cancel = orderService.selectOrderCancel(member);
			change = orderService.selectOrderChange(member);
			reaper = orderService.selectOrderReaper(member);
			
			logger.debug("cancel >> " + cancel);
			logger.debug("change >> " + change);
			logger.debug("reaper >> " + reaper);
		}catch(Exception e){
		...
		request.setAttribute("reaper", reaper);
